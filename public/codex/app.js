let CSV_SOURCES = {
  contratos: localStorage.getItem('urlContratos') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQqrB9Vyai3BbY6qNgReC91xpD4ZETXN3s273e1bY_9ysp_78U4boSvLaQjBgtGgUKlqXJBB8bdRk2w/pub?gid=371113084&single=true&output=csv',
  entregas: localStorage.getItem('urlEntregas') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQsJ46qpuqVO3VvQLp9R3sLBJe7a5vLu02ae9nox4hyc4t9rnUAr74B3fqCA5dRmGyt6rDcuogbwvvU/pub?gid=0&single=true&output=csv',
  coletas: localStorage.getItem('urlColetas') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQp8ZBJdS-SiRZQgUpUuycrORfLC1JzzkIjz2aGLqFELs9qbg1RMLPWdFtgmvaC6UuGt1SVKPv6ysC3/pub?gid=0&single=true&output=csv',
  veiculos_diario: localStorage.getItem('urlVeiculosDiario') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQybI5KTowf4OiZci5-hCkN7iX4nx0ZVS1oFxOB9H2Bxm8Um4z3tiqtn9lhvl4iByxISR3Hr4qxxTx0/pub?gid=360491932&single=true&output=csv',
  veiculos_previa: localStorage.getItem('urlVeiculosPrevia') || 'https://docs.google.com/spreadsheets/d/e/2PACX-1vQybI5KTowf4OiZci5-hCkN7iX4nx0ZVS1oFxOB9H2Bxm8Um4z3tiqtn9lhvl4iByxISR3Hr4qxxTx0/pub?gid=1596383573&single=true&output=csv',
};
...
function filterData(data) {
  ...
  return data.filter((row) => {
    const d = row.day;
    const periodPass = (!startDate || d >= startDate) && (!endDate || d <= endDate);
...
function setupFilters() {
  ['startDate', 'endDate', 'courierFilter', 'vehicleFilter', 'contractFilter'].forEach((id) => {
...
async function init() {
  ...
  try {
    ...
    const [contracts, deliveries, pickups, veiculosDiario, veiculosPrevia] = await Promise.all([
      fetchCsv(CSV_SOURCES.contratos),
      fetchCsv(CSV_SOURCES.entregas),
      fetchCsv(CSV_SOURCES.coletas),
      fetchCsv(CSV_SOURCES.veiculos_diario),
      fetchCsv(CSV_SOURCES.veiculos_previa),
    ]);

    contractsData = contracts;
    vehiclesData = [...veiculosDiario, ...veiculosPrevia];
...
function setupConfigModal() {
  ...
  const btnConfigSidebar = document.getElementById('btnConfigSidebar');
  const closeConfig = document.getElementById('closeConfig');
  const saveConfig = document.getElementById('saveConfig');

  const openModal = () => {
    document.getElementById('urlContratos').value = CSV_SOURCES.contratos;
    document.getElementById('urlEntregas').value = CSV_SOURCES.entregas;
    document.getElementById('urlColetas').value = CSV_SOURCES.coletas;
    document.getElementById('urlVeiculosDiario').value = CSV_SOURCES.veiculos_diario;
    document.getElementById('urlVeiculosPrevia').value = CSV_SOURCES.veiculos_previa;
  ...
  const saveAndReload = () => {
    localStorage.setItem('urlContratos', document.getElementById('urlContratos').value);
    localStorage.setItem('urlEntregas', document.getElementById('urlEntregas').value);
    localStorage.setItem('urlColetas', document.getElementById('urlColetas').value);
    localStorage.setItem('urlVeiculosDiario', document.getElementById('urlVeiculosDiario').value);
    localStorage.setItem('urlVeiculosPrevia', document.getElementById('urlVeiculosPrevia').value);
    window.location.reload();
  };

  if (btnConfig) btnConfig.addEventListener('click', openModal);
  if (btnConfigSidebar) btnConfigSidebar.addEventListener('click', openModal);
  closeConfig.addEventListener('click', closeModal);
  saveConfig.addEventListener('click', saveAndReload);
}
